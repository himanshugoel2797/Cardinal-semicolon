...
#ifndef CARDINAL_ELF_TYPES_H
#define CARDINAL_ELF_TYPES_H
...
/**
 * elf_header_valid - validate a 64-bit ELF header.
 * @hdr:           pointer to the Elf64_Ehdr (via its Elf_CommonEhdr).
 * @expected_type: the required e_type value (e.g. ET_REL or ET_EXEC).
 *
 * Checks the ELF magic, little-endian data encoding, e_type, e_machine
 * (non-EM_NONE), e_version, and OSABI (ELFOSABI_NONE or ELFOSABI_GNU).
 *
 * Returns 1 if all checks pass, 0 otherwise.
 */
static inline int elf_header_valid(const Elf64_Ehdr *hdr, uint16_t expected_type)
{
    const Elf_CommonEhdr *c_hdr = &hdr->e_hdr;
    ...
    if (c_hdr->e_ident[EI_DATA] != ELFDATA2LSB)
        return 0;
    if (c_hdr->e_type != expected_type)
        return 0;
    if (c_hdr->e_machine == EM_NONE)
        return 0;
