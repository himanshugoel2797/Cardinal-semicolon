...
static uint64_t ms_100000000x = 0, ms_sinceboot = 0;
...
static void rtc_interrupt(int int_num)
{
    ...
    if (int_mask & 0x40)
        ms_100000000x += 9765625; //increment by the period of the interrupt
    if (ms_100000000x >= 100000000)
    {
        seconds++;
        ms_100000000x -= 100000000;
        ms_sinceboot++;
    }
    if (seconds >= 60)
    {
...
PRIVATE int rtc_init()
{
    ...
    if (interrupt_allocate(1, interrupt_flags_fixed | interrupt_flags_exclusive, &irq) == 0)
    {
        ...
        DEBUG_PRINT("[SysTimer] RTC Initialized\r\n");
        return 0;
    }
