 ...
 * (https://github.com/kokke/tiny-AES128-C/pull/12),
 ...
 *  up to rcon[8] for AES-192, up to rcon[7] for AES-256. rcon[0] is not used in
 * AES algorithm."
 *
...
static void XorWithIv(uint8_t *buf) {
    uint8_t i;
    for (i = 0; i < BLOCKLEN; ++i) // WAS for(i = 0; i < KEYLEN; ++i) but the
        // block in AES is always 128bit so 16 bytes!
    {
        buf[i] ^= Iv[i];
    }
}
