...
struct bcchunk {  // opaque typedef in lbc.h
    ...
    rinstr *rcode;     // register-form code
    int nrcode, caprcode;
...
// ===========================================================================
// Register-bytecode compiler + VM. Instructions name operand + destination
// registers explicitly, so a local read is free (the op addresses the local's
// register directly) and `(+ x y)` is ONE dispatch. This is the sole evaluator;
// it reuses the shared scope/resolve/const/global-slot/thin-call machinery above
// and is fuzz/corpus-tested on the host against the ctx-VM oracle (test_bytecode).
// ===========================================================================
