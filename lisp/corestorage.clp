          ...
          (cond
            ((eq? (car m) 'register-blockdev)    ; (... name bsize bcount driver)
             (let ((dev (list (cadr m) (caddr m) (cadddr m) #f (nth m 4))))
               (display "[corestorage] block device registered: ")
