        ...
        (let* ((psc (e-r32 mmio (+ op (EOP-PORTSC i))))
          ...
          (cond
            ((and connected (not was))
             (display "[ehci] device connected, port ") (display i) (newline)
             (let ((res (ehci-enable-port! mmio op i)))
               (if (eq? res 'high)
                   (begin (display "[ehci] port ") (display i) (display " high-speed; enumerating") (newline)
                          (send usb (list 'port-connected (self) i USB-SPEED-HIGH))
                          (loop (+ i 1) (cdr pe) (cons #t acc)))
                   (begin (display "[ehci] port ") (display i) (display " ") (display res)
                          (display " (released/none)") (newline)
                          (loop (+ i 1) (cdr pe) (cons #f acc))))))
            ((and (not connected) was)
             (display "[ehci] device disconnected, port ") (display i) (newline)
...
(define (ehci-bringup bar ecam usb)
  (let* ((mmio (mmio-map bar 4096))
    ...
    (display "[ehci] reset complete; nports=") (display nports) (display "; serving") (newline)
    (let pp ((i 0))
      (if (< i nports)
...
(define (ehci-init usb)
  (let ((ecam (find-ehci EHCI-IDS)))
    (if (not ecam)
        (begin (display "[ehci] no controller present") (newline) #f)
        (let ((cfg (mmio-map ecam 4096)))
          ...
          (let ((bar (let ((b (bar-base cfg 0)))
            ...
            (if (or (not bar) (= bar 0))
                (begin (display "[ehci] no BAR") (newline) #f)
                (begin (display "[ehci] bar=") (display bar) (newline)
